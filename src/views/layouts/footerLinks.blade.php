<script>
    ...
    let ip_address = "{{config('devschat.socket.host')}}";
    let socket_port = "{{config('devschat.socket.port')}}";
    ...
    access_token = $('meta[name="csrf-token"]').attr('content'),
        ...
        socket.on('userOnline',function (member,username) {
            setActiveStatus(1, member);
            $('.messenger-list-item[data-contact=' + member + ']').find('.activeStatus').remove();
            $('.messenger-list-item[data-contact=' + member + ']').find('.avatar').before(activeStatusCircle());
        });

        socket.on('user-disconnect',function (user_id,user_name) {
            setActiveStatus(0, user_id);
            console.log('User Disconnect:'+user_id);
            $('.messenger-list-item[data-contact=' + user_id + ']').find('.activeStatus').remove();
        });
        // socket.on('')
            ...
            socket.on('update-conversation',function (data) {
                if (data.receiver_id == auth_id) {
                    fetchMessages(data.sender_id, messenger.split('_')[0]);
                    // console.log(messenger.split('_')[1], messenger.split('_')[0]);
                }
            })

            socket.on('client-contactItem',function (data) {
                if (data.receiver_id == auth_id) {
                    data.updating == true ? updateContatctItem(data.sender_id)
